...
You should be able to see the contract starting in the monitoring window. There will be two containers running, one for winenftmsp and one for org2. The container names contain the organization/peer and the name of the chaincode.
...
export CORE_PEER_LOCALMSPID="winenftmsp"
export CORE_PEER_TLS_ROOTCERT_FILE=${PWD}/organizations/peerOrganizations/winenftmsp.example.com/tlsca/tlsca.winenftmsp.example.com-cert.pem
export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/winenftmsp.example.com/users/Admin@winenftmsp.example.com/msp
export CORE_PEER_ADDRESS=localhost:7051
export PATH=${PWD}/../bin:$PATH
export FABRIC_CFG_PATH=${PWD}/../config

# invoke the function
peer chaincode query -C canal-staging -n basicts -c '{"Args":["org.hyperledger.fabric:GetMetadata"]}' | jq
```
...
```bash
    docker run --rm -d --name peer0winenftmsp_assettx_ccaas  \
                  --network fabric_test \
...
```bash
 docker run --rm -it --name peer0winenftmsp_basicj_ccaas -p 8000:8000 --network fabric_test -e DEBUG=true -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:9999 -e CHAINCODE_ID=basicj_1.0:b014a03d8eb1898535e25b4dfeeb3f8244c9f07d91a06aec03e2d19174c45e4f -e CORE_CHAINCODE_ID_NAME=basicj_1.0:b014a03d8e
...
In the peer's environment configuration we then set for winenftmsp's peer1

```bash
CHAINCODE_AS_A_SERVICE_BUILDER_CONFIG="{\"peername\":\"winenftmsppeer1\"}"
```

The external builder will then resolve this address to be `winenftmsppeer1_assettransfer_ccaas:9999` for the peer to use.
