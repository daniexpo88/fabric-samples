...
function printHelp() {
  ...
  if [ "$USAGE" == "up" ]; then
    ...
    println "    -c <channel name> - Name of channel to create (defaults to \"canal-staging\")"
    ...
    println
    println " Examples:"
    println "   network.sh up createChannel -ca -c canal-staging -s couchdb "
  elif [ "$USAGE" == "createChannel" ]; then
    println "Usage: "
    println "  network.sh \033[0;32mcreateChannel\033[0m [Flags]"
    println
    println "    Flags:"
    println "    -c <channel name> - Name of channel to create (defaults to \"canal-stagings\")"
    println "    -r <max retry> - CLI times out after certain number of attempts (defaults to 5)"
    println "    -d <delay> - CLI delays for a certain number of seconds (defaults to 3)"
    println "    -verbose - Verbose mode"
    println
